...
public class OrdersController : ControllerBase
{
    ...
    [Route("complete")]
    [HttpPost]
    public async Task<ActionResult> CompleteOrderAsync([FromBody] CompleteOrderCommand completeOrderCommand)
    {
        _logger.LogInformation($"Sending complete order command:{completeOrderCommand.GetGenericTypeName()}-{nameof(completeOrderCommand.OrderNumber)}:{completeOrderCommand.OrderNumber}({completeOrderCommand})");

        var completeCommandResut =  await _mediator.Send(completeOrderCommand);
        return completeCommandResut ? Ok() : BadRequest();
    }
}
